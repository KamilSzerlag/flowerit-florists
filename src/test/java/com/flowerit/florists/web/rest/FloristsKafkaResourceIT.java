...
import com.flowerit.florists.config.KafkaProperties;
...
class FloristsKafkaResourceIT {
    ...
    void setup() {
        ...
        FloristsKafkaResource kafkaResource = new FloristsKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/florists-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/florists-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
